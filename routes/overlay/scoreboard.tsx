...
// existem nesta rota). Cor de destaque entra por --accent (style inline, vindo das settings);
// o resto é hex via <style> injetado. Tamanhos em px: a fonte de navegador do OBS tem resolução
// fixa (o operador define, ex 1920x1080). Escala pensada pra ~1920px de largura de cena.
const CSS = `
  ...
  /* pílula flutuante: estado da conexão + relógio + etiqueta */
  .el-chip {
    display: flex; align-items: center; gap: 12px;
    margin-bottom: 104px;                 /* folga pro medalhão que sobe acima da barra */
  ...
  .el-dot.off { background: #ef4444; box-shadow: 0 0 0 4px rgba(239,68,68,0.20); animation: none; }
  .el-clock {
  ...
  .el-chip-sep { width: 1px; height: 20px; background: rgba(255,255,255,0.18); }
  ...
  /* filete de destaque na base da barra */
  .el-bar::after {
  ...
  /* placa central com os dois números; segmento do meio da barra, cantos retos de propósito */
  .el-plate {
    display: flex; align-items: center;
    padding: 26px 96px;
  ...
  .el-num.home { margin-right: 78px; }
  .el-num.away { margin-left: 78px; }

  /* medalhão com a logo da liga, centralizado, pousado sobre o topo da barra */
  .el-medallion {
  ...
  .el-logo { width: 82%; height: 82%; object-fit: contain; filter: drop-shadow(0 3px 7px rgba(0,0,0,0.28)); }
  .el-monogram { font-size: 42px; font-weight: 900; letter-spacing: 1px; color: #0b7a37; }
...
export function Scoreboard({
  ...
  const showChip = !live || showClock || state.label.length > 0;
      ...
      <div className="el-wrap" style={{ "--accent": accentColor } as CSSProperties}>
        {showChip ? (
          <div className="el-chip">
            ...
            {!live ? <span className="el-chip-label">Sem sinal</span> : null}
            {showClock ? (
              <span className={`el-clock ${state.clock.running ? "run" : ""}`}>{formatClock(elapsed)}</span>
            ) : null}
            {live && showClock && state.label ? <span className="el-chip-sep" /> : null}
            {live && state.label ? <span className="el-chip-label">{state.label}</span> : null}
          </div>
        ) : null}
        ...
        <div className="el-bar">
          ...
          <div className="el-medallion" aria-hidden="true">
            {logoOk && logoUrl ? (
